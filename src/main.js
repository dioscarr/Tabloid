  ...
  <div class="min-h-screen bg-slate-950 text-slate-100">
    ...
    <div class="lg:pl-72">
      ...
      <main class="mx-auto max-w-[1500px] px-4 py-7 sm:px-7 lg:py-9">
        ...
        <section class="mt-6 rounded-2xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 via-slate-900/70 to-violet-500/5 p-5 sm:p-7"><div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div class="max-w-2xl"><div class="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-indigo-300">${icon('apps', 'size-4')} Development workspaces</div><h3 class="mt-3 text-2xl font-black tracking-tight">Launch VS Code with the repository ready.</h3><p class="mt-2 text-sm leading-6 text-slate-400">Choose a branch, create an isolated private code-server workspace, and continue interactively with Git and Copilot. Delegated agent work stays issue- and pull-request-driven.</p><div class="mt-4 flex flex-wrap gap-2 text-xs text-slate-400"><span class="rounded-full border border-slate-700 px-3 py-1.5">Private Tailscale URL</span><span class="rounded-full border border-slate-700 px-3 py-1.5">Persistent workspace</span><span class="rounded-full border border-slate-700 px-3 py-1.5">24-hour expiry</span><span class="rounded-full border border-slate-700 px-3 py-1.5">Scoped Git access</span></div></div><button id="workspace-button" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-indigo-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-400">${icon('plus')} Create workspace</button></div></section>
      </main>
    </div>
...
document.querySelector('#workspace-button')?.addEventListener('click', () => { workspaceDialog?.classList.remove('hidden'); workspaceDialog?.classList.add('flex') })
