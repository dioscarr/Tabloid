...
<section class="workspace"><div class="section-head"><div><p class="kicker">Neural topology</p><h2>Connection map</h2><p>Select a node to trace every route in and out.</p></div><div class="topology-controls"><div class="view-switch" role="group" aria-label="Topology view"><button data-topology-view="2d" class="active">2D map</button><button data-topology-view="3d">3D brain</button></div><div class="filters" role="group"><button data-filter="all" class="active">All</button><button data-filter="healthy">Healthy</button><button data-filter="warning">Attention</button></div></div></div><div id="topology-2d" class="map-grid"><div class="graph"><div class="graph-toolbar"><span><i class="legend healthy"></i> Healthy</span><span><i class="legend warning"></i> Attention</span><span class="prototype">Prototype telemetry</span></div><svg id="lines" class="lines" viewBox="0 0 1000 620" preserveAspectRatio="none"></svg><div id="nodes" class="nodes"></div></div><aside id="inspector" class="inspector" aria-live="polite"></aside></div><div id="topology-3d" class="topology-3d" hidden><div id="topology-3d-scene"></div><button id="topology-3d-reset" class="topology-3d-reset" type="button">Reset view</button></div></section>
...
function renderTopologyView(){
  ...
  if(topologyMode==='3d'&&!topology3d){topology3d=mountTopology3D({container:document.querySelector('#topology-3d-scene'),apps,routes,getMetric:routeMetric,onSelect:select});document.querySelector('#topology-3d-reset').onclick=()=>topology3d.reset()}
}
